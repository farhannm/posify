<x-app-layout title="Edit Cashier" is-header-blur="true">
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        ...
        <div class="mt-6 flex flex-col items-center justify-between space-y-2 text-center sm:flex-row sm:space-y-0 sm:text-left">
            <div class="flex items-center space-x-4 py-5 lg:py-6">
                ...
                <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                    <li class="flex items-center space-x-2">
                        <a class="text-primary hover:underline" href="{{ route('edit-cashier-form', $users->id) }}">Edit Cashier</a> 
                    </li>
                </ul>
            </div>
        </div>

        <form action="{{ route('cashier-update', $users->id) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
                                ...
                                <div class="space-y-4 p-4 sm:p-5">
                                    <label class="block">
                                        ...
                                        <input
                                            ...
                                            placeholder="Enter fullname..." type="text" value="{{ $users->fullname }}" required />
                                    </label>
                                    <label class="block">
                                        ...
                                        <input
                                            name="email"
                                            class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                            placeholder="Enter email..." type="email" value="{{ $users->email }}" required />
                                    </label>
                                </div>
                            </div>
