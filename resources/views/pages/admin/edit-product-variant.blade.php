<x-app-layout title="Edit Variants" is-header-blur="true">
    ...
    <main class="main-content w-full px-[var(--margin-x)] pb-8">
        <div class="flex items-center space-x-4 py-5 lg:py-6">
            ...
            <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                <li class="flex items-center space-x-2">
                    <p class="text-primary">Edit Variants</p> 
                </li>
            </ul>
        </div>

        <form action="{{ route('edit-variants', ['id' => $product->id, 'variant_id' => $selectedVariant->id]) }}" enctype="multipart/form-data" method="POST">
            @csrf
            @method('PUT')
                        ...
                        @foreach($variant as $item)
                            <li>
                                <input type="checkbox" 
                                    ...
                                    value="{{ $item->id }}" 
                                    class="hidden peer" 
                                    {{ collect($selectedVariantCombinations)->contains(function ($combination) use ($item) {
                                        return in_array($item->id, $combination);
                                    }) ? 'checked' : '' }}>
                ...
                <div class="grid grid-cols-2 gap-4 mt-4">
                    <div class="grid grid-cols-1">
                        <label class="block">
                            ...
                            <input
                                name="stock"
                                value="{{ old('stock', $selectedVariant->stock) }}"
                    ...
                    <div class="grid grid-cols-1">
                        <label class="block">
                            ...
                            <input
                                ...
                                value="{{ old('additional_price', $selectedVariant->additional_price) }}" 
                                class="form-input mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
